...
# AAPL GOOGL GOOG MSFT META NVDA LLY MRK SMCI
# TQQQ QLD SOXL SPUU SPXL UPRO
# IBM SMCI JPM
input_input_string = "SMCI"
input_input_string = input_input_string.split()
for ist in input_input_string:
  input_string = "NVDA "
  input_string  += ist
  stocklist = input_string.split()
  # print(stocklist)
  numOfRandPort = 100000 # of runs
  # 2000000000
  # BRK-B AAPL MSFT AMZN TSLA PEP COST GOOGL NVDA JPM
  # initial_frequency = get_cpu_frequency()
  startTimer = dt.datetime.now()
  steve = optimalPortfolio(stocklist, numOfRandPort)
  print(steve.optRiskyPort(return_risk_free_rate()))
  # max_frequency = get_cpu_frequency()
  endTimer = dt.datetime.now() -  startTimer
  print(endTimer)
...
# print(f"Maximum CPU frequency (GHz) used: {max_frequency:.2f}")
# # Calculate the difference with the initial frequency
# frequency_difference = max_frequency - initial_frequency
# print(f"Frequency difference (GHz): {frequency_difference:.2f}")
