...
def trade_SPY():
    ...
    while True:
        ...
        if recent_price is not None:
            if current_price <= recent_price - Decimal('0.01'):
                ...
                if buyQty > 0 and (OrdersResponse.text == "[]"):
                    ...
                    buy_sell_QtyList_newAddition = str(buy_sell_QtyList_index) + f' Buy Order placed: {buyQty} shares at ${limBuy_Price}'
                    buy_sell_QtyList_index += 1
                    buy_sell_QtyList.append(buy_sell_QtyList_newAddition)
            elif current_price >= recent_price + Decimal('0.01'):
                OrdersResponse = requests.get(OrdersURL, headers=OrdersHeaders)
                if sellQty > 0 and (OrdersResponse.text == "[]"):
                    ...
                    buy_sell_QtyList_newAddition = str(buy_sell_QtyList_index) + f' Sell Order placed: {sellQty} shares at ${limSell_Price}'
                    buy_sell_QtyList_index += 1
                    buy_sell_QtyList.append(buy_sell_QtyList_newAddition)
